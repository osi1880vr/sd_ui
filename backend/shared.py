...
def save_last_prompt(prompt_html, prompt_txt):
    f = open('configs/aiPixels/last_prompt.txt', mode="w", encoding="utf-8")
    f.write(prompt_html)
    f.close()
    f = open('configs/aiPixels/prompt_history.txt', mode="a", encoding="utf-8")
    dt = datetime.now()
    f.write(f'{dt}: {prompt_html}\n')
    f.close()
    f = open('configs/aiPixels/prompt_history_raw.txt', mode="a", encoding="utf-8")
